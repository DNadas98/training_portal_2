...
public interface QuestionnaireSubmissionDao extends JpaRepository<QuestionnaireSubmission, Long> {
  ...
  @Query(
    value =
      "SELECT DISTINCT new net.dnadas.training_portal.dto.group.project.questionnaire.QuestionnaireSubmissionStatsInternalDto(" +
        ...
        "u.id, u.username, u.coordinatorUsername, u.dataPreparatorUsername, u.hasExternalTestQuestionnaire, u.hasExternalTestFailure, " +
        ...
        "AND (LOWER(u.username) LIKE %:searchValue% OR LOWER(u.coordinatorUsername) LIKE %:searchValue% OR LOWER(u.dataPreparatorUsername) LIKE %:searchValue%) " +
  ...
  @Query(
    ...
    countQuery = "SELECT COUNT(DISTINCT u.id) " +
      "FROM Project p " +
      "INNER JOIN p.assignedMembers u ON (LOWER(u.username) LIKE %:searchValue% OR LOWER(u.coordinatorUsername) LIKE %:searchValue% OR LOWER(u.dataPreparatorUsername) LIKE %:searchValue%) " +
