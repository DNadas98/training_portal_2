...
export default function UserJoinRequests() {
  ...
  return (
    <Grid container alignItems={"center"} justifyContent={"center"}> <Grid item xs={10}> <Stack
      spacing={2}>
      <Card elevation={10}>
        <CardHeader title={localized("pages.user.requests.group_title")}
                    titleTypographyProps={{variant: "h5"}}/>
        <CardContent>{groupJoinRequestsLoading ? <LoadingSpinner/> : !groupJoinRequests?.length
          ? <Typography
            variant={"body1"}>{localized("pages.user.requests.group_not_found")}</Typography>
          : <List>{groupJoinRequests.map(request => {
            return <ListItem key={request.requestId}><Card elevation={10} sx={{width: "100%"}}>
              <CardContent><Stack spacing={1}>
                ...
                <Button sx={{maxWidth: "fit-content"}} color={"error"} variant={"contained"}
                        onClick={async () => {
      ...
      <Card elevation={10}>
        <CardHeader title={localized("pages.user.requests.project_title")}
                    titleTypographyProps={{variant: "h5"}}/>
        <CardContent>{projectJoinRequestsLoading ? <LoadingSpinner/> : !projectJoinRequests?.length
          ? <Typography
            variant={"body1"}>{localized("pages.user.requests.project_not_found")}</Typography>
